...
class DatabaseExportNotifier extends _$DatabaseExportNotifier {
  ...
  Future<void> createIsarExportFile() async {
    try {
      ...
      state = StateOf<File?>.failure(AppException.fromError(err));
    }
  }
  ...
  Future<void> shareExportedDatabaseFile() async {
    ...
    if (!File(filePath).existsSync()) {
      return dialogService.showSnackBar(S.current.fileNotFound);
    }

    await dialogService
        .shareFile([filePath], 'Share Exported Database').then((result) {
      result.when(
        failure: (l) => dialogService.showInfoDialog(l.toString()),
        success: (r) {
